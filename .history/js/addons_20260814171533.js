/* =============================================
   ...
   Version 2.2 - Auto-detect + Multi-source Lookup + Type Categories
   ============================================= */

/* NOTA SOBRE FUENTES DE METADATA:
   No se usa minecraft.net/marketplace porque:
   1) No expone una API publica ni permite fetch() desde otro dominio (CORS cerrado,
      todo se carga con servicios internos autenticados de Microsoft).
   2) El Marketplace oficial solo lista DLC de pago de partners de Mojang; los addons
      de esta pagina son mods de la comunidad, asi que ahi nunca apareceria nada.
   En su lugar se consultan dos fuentes que SI tienen datos de addons de fans:
   - BedrockExplorer (API + paginas con og:meta)
   - MCPEDL (API REST de WordPress, tambien con datos de addons/mundos/texturas/shaders)
   Si ninguna responde, se usa una imagen y descripcion generadas por categoria. */
...
async function searchMCPEDL(query) {
  try {
    const url = 'https://mcpedl.com/wp-json/wp/v2/posts?search=' +
      encodeURIComponent(query) + '&per_page=1&_embed';
    const res = await fetch(url, { signal: AbortSignal.timeout(5000) });
    ...
    return {
      name: stripHtml(post.title && post.title.rendered),
...
// MARKETPLACE SEARCH - prueba varias fuentes en cascada hasta encontrar una
// coincidencia con imagen real. Cachea el resultado (positivo o negativo) por sesion.
async function searchMarketplace(query) {
  const cacheKey = 'mkt:' + query.toLowerCase();
  const cached = getCache()[cacheKey];
  if (cached !== undefined) return cached;

  let result = await searchBedrockExplorer(query);
  if (!result || !result.imageUrl) {
    result = await searchMCPEDL(query);
  }
...
function renderControlPanel(container, addonsCount) {
  ...
  // Fila 1: tipo de contenido (Addons / Mundos / Skins / Texturas / Shaders)
  const presentTypes = [...new Set(currentAddons.map(a => a.type))];
  const typeOrder = ['addon', 'world', 'skin', 'texture', 'shader'].filter(t => presentTypes.includes(t));
  const typeFilterHtml = ['Todos', ...typeOrder].map(t => {
    const meta = TYPE_META[t] || { icon: '\ud83d\udd39', label: t };
    const activeClass = t === activeTypeFilter ? 'active' : '';
    return '<button class="filter-btn ' + activeClass + '" onclick="filterByType(\'' + t + '\')">' + meta.icon + ' ' + meta.label + '</button>';
  }).join('');

  // Fila 2: genero (Terror, Aventura, Survival...), solo dentro del tipo activo
  const genreIcons = {
  ...
  panel.innerHTML =
    ...
    '</div>' +
    '<div class="addon-filters addon-filters-type">' + typeFilterHtml + '</div>' +
    (genreFilterHtml ? '<div class="addon-filters addon-filters-genre">' + genreFilterHtml + '</div>' : '') +
...
// FILTER - aplica el filtro de tipo Y el de genero combinados
function applyFilters() {
  document.querySelectorAll('.addon-card').forEach(function(card) {
    ...
    const matches = matchesType && matchesGenre;
